...
def load_articles(articles_dirname, gloves):
    ...
    for file in files[1:]:
        file_text = get_text(file)
        file_text = file_text.split('\n')
        title = file_text[0]
        text = ' '.join(file_text[1:])
        # print(doc2vec(text,gloves))
        la.append([file, title, text, doc2vec(text, gloves)])
    return la
...
def distances(article, articles):
    ...
    """
    return [(np.linalg.norm(article_x[3] - article[3]), article_x) for article_x in articles if article_x != article]
...
def recommended(article, articles, n):
    ...
    sorted_list = sorted(dist, key=lambda x: x[0])[0:n]
    return [art[1] for art in sorted_list]
...
if __name__ == '__main__':
    ...
    print(recommended(articles[3], articles,3))
    #print(articles[2])
